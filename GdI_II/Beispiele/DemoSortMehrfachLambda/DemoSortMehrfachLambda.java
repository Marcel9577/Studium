...
{ 
  ...
  { 
    ...
    ArrayList<Freund> eineListe = 
      new ArrayList<Freund>();
    ...
    List<Freund> aufsteigendesAlter = 
      eineListe.stream()
        .sorted((freund1, freund2) 
          -> freund1.alterUnterschied(freund2))
        .collect(toList());
      
    druckeFreunde("Sortierung nach aufsteigendem Alter:", 
      aufsteigendesAlter);    
    ...
    druckeFreunde("Sortierung nach absteigendem Alter:", 
      aufsteigendesAlter);
    ...
    List<Freund> aufsteigendNachnamen = 
      eineListe.stream()
        .sorted((freund1, freund2) -> 
         freund1.getNachname().compareTo(
          freund2.getNachname()))
        .collect(toList());
      
    druckeFreunde("Sortierung nach aufsteigendem Nachnamen:", 
      aufsteigendNachnamen); 
    
    //Version (6) - Mit funktionaler Schnittstelle
    final Function<Freund, String> nachName = 
      freund -> freund.getNachname();
 
    aufsteigendNachnamen =
      eineListe.stream().sorted(comparing(nachName))
      .collect(toList());
    ...
    final Function<Freund, Integer> nachAlter = 
      freund -> freund.getAlter();
 
    druckeFreunde("Sortierung aufsteigend zuerst nach Alter, " 
      + "dann nach Nachnamen: ",
      eineListe.stream()
        .sorted(comparing(nachAlter).thenComparing(nachName))
        .collect(toList())); 
  }

  private static void druckeFreunde
    (String titel, List<Freund> freunde) 
  {     
