...
public class ObjektContainer
{
    ...
    {
        if (einObjektContainer == null) einObjektContainer = new ObjektContainer();
        ...
        Character status, Person kontaktperson)
    {
        ...
        Prioritaet p = null;
        if(prioritaet!=null) p=enumPrioritaet.get(prioritaet);
        Kategorie k = null;
        if(kategorie!=null) k=enumKategorie.get(kategorie);
        Status s = null;
        if(status!=null) s=enumStatus.get(status);
    ...
    public Iterator<Aufgabe> iteratorAufgaben(Character aufgabensortierung)
    {
        ...
        {
            einComparator = Aufgabe.enumAufgabensortierung.get(
                    enumAufgabensortierung.get(aufgabensortierung));    
        }
    ...
    public Iterator<Person> iteratorKontaktpersonen()
    {
        try
        {
